...
describe("e2e-in-memory", () => {
  ...
  describe("InMemCache", () => {
    ...
    it("should bust all param based cache with a key if bustAllChildren is true", async () => {
      ...
      // no param
      const start = Date.now();
      const response = await request(httpServer).get("/test3/noParam");
      const diff = Date.now() - start;
      ...
      const response3 = await request(httpServer).get("/test3/noParam");
      ...
      const response6 = await request(httpServer).get("/test3/noParam");
    ...
    it("should work with object parameters", async () => {
      ...
      equal(
        response3.text,
        "test3" + Object.keys(modifiedRequestBody).join("")
      );
    });
    ...
    it('should bust all addition cache if "addition" option is specified when bust', async () => {
      await request(httpServer).get("/test6");
      await request(httpServer).get("/under_test6");
      await request(httpServer).patch("/test6/bust");
